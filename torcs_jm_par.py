...
try:
    with open('best_line.json', 'r') as f:
        BEST_TRAJECTORY = json.load(f)
    print("Trazada cargada.")
except FileNotFoundError:
    BEST_TRAJECTORY = None
    print("Aviso: No hay best_line.json, se usará conducción estándar.")
...
TARGET_SPEED = 35  # Target speed in km/h. Increasing this makes the car go faster but may reduce stability. (normal: 100)
STEER_GAIN = 10     # Steering sensitivity. Higher values make the car turn more aggressively.
CENTERING_GAIN = 0.05  # How strongly the car corrects its position toward the center of the track.
BRAKE_THRESHOLD = 0.5  # Angle threshold for braking. Lower values brake earlier.
GEAR_SPEEDS = [0, 10, 20, 40, 50, 180]  # Speed thresholds for gear shifting. (para volver a normal duplicar)
